import { useReps } from './utils.js'

export default function RepsList() {
  const { data, isLoading, isError } = useReps()
  ...
  if (data) return (
    <div className="mx-10">
      <h1 className="text-xl mt-5">Sales Reps</h1>
      <div className="mt-3 flex flex-col">
        <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                ...
                <tbody>
                  {data.map((item, itemIdx) => (
                    <tr key={item.id} className={itemIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.role.name}</td>
            ...
            {data.length === 0 && 
              <div className='mt-5 text-center text-gray-400'>You don't have any Reps yet.</div>
            }
          </div>
        </div>
      </div>
